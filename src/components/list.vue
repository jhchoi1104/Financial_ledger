<template>
    <div class="d-flex align-items-center gap-2">
        <input type="date" id="date-input" v-model="date1" class="form-control" @change="changeDate(date1)" style="width: 150px; height: 38px" />
    ...
    <table class="table table-hover">
        ...
        <tbody>
            <tr v-for="item in list" :key="item.id">
                ...
                <td>{{ item.category }}</td>
                <td>{{ item.type }}</td>
                <td v-if="item.type === '지출'" style="color: #e0115f">₩ {{ formatAmount(item.amount) }}</td>
                <td v-else style="color: steelblue">₩ {{ formatAmount(item.amount) }}</td>
                <td>{{ item.memo }}</td>
                <td>
                    <button
                        type="button"
                        ...
                        style="margin-left: 60px; font-size: 0.9rem; padding: 0px 10px 20px; width: 35px; height: 35px"
                    >
                    ...
                    <button
                        ...
                        style="margin-left: 5px; font-size: 0.9rem; padding: 0px 10px 20px; width: 35px; height: 35px"
...
const { chg_edit, deleteList, optionFiltering, selectList, changeDate, selectedValue, selectionOption, date1 } = useTodoListStore();
